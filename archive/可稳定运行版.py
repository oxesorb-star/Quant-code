...
MODEL_R1 = "deepseek-r1:14b"  # 深度分析模型
...
async def httpx_ask_ollama(model, prompt, temperature=0.2, force_json=False):
    ...
    try:
        ...
        log_terminal("AI 超时", f"模型 {model} 在 90s 内未完成推理 | 请求参数：temperature={temperature}, force_json={force_json}")
...
def update_balance(acc_type, amount_change, reason=""):
    ...
    try:
        ...
        data = {'cash': new_cash, 'initial_capital': initial_capital, 'last_update': datetime.now().strftime("%Y-%m-%d %H:%M:%S")}

        atomic_write_json(balance_file, data)

        log_terminal("余额更新", f"{'💰' if acc_type == 'real' else '🧪'} {acc_type} 余额: {cash:+.2f} -> {new_cash:+.2f} | 原因: {reason}")
        return new_cash
    except Exception as e:
    ...
    if os.path.exists(AI_DECISIONS_FILE):
        ...
        for d in reversed(decisions[-50:]):
            ...
            try:
                ...
                continue
            if hours_passed < 24 or hours_passed > 72:
                continue
            # 通过实时价格回溯
            price_map = get_price_map()
            current_price = price_map.get(d["code"], d.get("price", 0))
            if current_price and d.get("price", 0) > 0:
                change_pct = (current_price - d["price"]) / d["price"] * 100
                if change_pct > 1:
                    lessons.append(
                        f"💡 {d.get('name', d['code'])} {d['code']} | 未采纳 | "
                        f"次日{'+' if change_pct > 0 else ''}{change_pct:.1f}% | 错失机会"
                    )
                elif change_pct < -1:
                    lessons.append(
                        f"✅ {d.get('name', d['code'])} {d['code']} | 未采纳 | "
                        f"次日{change_pct:.1f}% | 幸运躲过"
                    )
    ...
    try:
        ...
        def _build_holding_summary(holdings, label):
            ...
            for code, info in holdings.items():
                ...
                cur_price = cached.get('price', info.get('buy_price', 0))
                if info.get('buy_price', 0) > 0:
                    pnl = (cur_price - info['buy_price']) * info['volume']
                    pnl_pct = (cur_price / info['buy_price'] - 1) * 100
                    sign = '+' if pnl >= 0 else ''
                    lines.append(f"    {code} {cached.get('name', code)}: {sign}{pnl:.0f}元 ({sign}{pnl_pct:.1f}%)")
            return '\n'.join(lines) + '\n'
            ...
            try:
                ...
                if curr_price <= dyn_stop:
                    ...
                    if time.time() - last_alert > ALERT_COOLDOWN_SECONDS:
                        ...
                        push_decision(f"💰实盘触发止损保护({dyn_label})", msg, code=code)
                        _alert_cooldown[f"stop_real_{code}"] = time.time()
                        _alert_cooldown[f"stop_recheck_real_{code}"] = time.time() + 1800
                    else:
                        log_terminal("盈亏检查", f"⏳ 实盘 {name} 触发止损({dyn_label})，但冷却中({int((ALERT_COOLDOWN_SECONDS-(time.time()-last_alert))/60)}分钟后可再推)")
                    ...
                    if recheck_key in _alert_cooldown and time.time() > _alert_cooldown[recheck_key]:
                        ...
                        push_decision("⚠️ 实盘止损延迟警告", msg2, code=code)
                ...
                if pnl >= 10.0:
                    ...
                    if gemma_score >= 36 and ds_conf == "高":
                        ...
                        if time.time() - last_alert > ALERT_COOLDOWN_SECONDS:
                            ...
                            push_decision("💰实盘触发第三止盈(双模型确认)", msg, code=code)
                            _alert_cooldown[f"tp3_real_{code}"] = time.time()
                        else:
                            log_terminal("盈亏检查", f"⏳ 实盘 {name} 触发tp3({pnl:.1f}%)，但冷却中")
                    else:
                        log_terminal("盈亏检查", f"📈 实盘 {name} 盈利{pnl:.2f}%超过10%，但AI置信度不足(Gemma:{gemma_score}/40, DS:{ds_conf})，跳过第三止盈推送")
                elif pnl >= 5.0:
                    last_alert = _alert_cooldown.get(f"tp2_real_{code}", 0)
                    if time.time() - last_alert > ALERT_COOLDOWN_SECONDS:
                        msg = f"🏆 实盘第二止盈提醒！\n标单: {name}({code})\n现价: {curr_price}\n成本: {buy_price}\n盈亏: {pnl:.2f}%\n🛡️ 保护止损已上移至: ¥{dyn_stop:.2f}({dyn_label})\n⚠️ 建议大幅减仓或清仓"
                        push_decision("💰实盘触发第二止盈", msg, code=code)
            ...
            try:
                ...
                if curr_price <= dyn_stop:
                    ...
                    if time.time() - last_alert > ALERT_COOLDOWN_SECONDS:
                        ...
                        push_decision(f"🧪模拟盘触发止损保护({dyn_label})", msg, code=code)
                        _alert_cooldown[f"stop_sim_{code}"] = time.time()
                        _alert_cooldown[f"stop_recheck_sim_{code}"] = time.time() + 1800
                    else:
                        log_terminal("盈亏检查", f"⏳ 模拟盘 {name} 触发止损({dyn_label})，但冷却中({int((ALERT_COOLDOWN_SECONDS-(time.time()-last_alert))/60)}分钟后可再推)")
                    ...
                    if recheck_key in _alert_cooldown and time.time() > _alert_cooldown[recheck_key]:
                        ...
                        push_decision("⚠️ 模拟盘止损延迟警告", msg2, code=code)
                        del _alert_cooldown[recheck_key]
                ...
                if pnl >= 10.0:
                    ...
                    if gemma_score >= 36 and ds_conf == "高":
                        ...
                        if time.time() - last_alert > ALERT_COOLDOWN_SECONDS:
                            ...
                            push_decision("🧪模拟盘触发第三止盈(双模型确认)", msg, code=code)
                            _alert_cooldown[f"tp3_sim_{code}"] = time.time()
                        else:
                            log_terminal("盈亏检查", f"⏳ 模拟盘 {name} 触发tp3({pnl:.1f}%)，但冷却中")
                    else:
                        log_terminal("盈亏检查", f"📈 模拟盘 {name} 盈利{pnl:.2f}%超过10%，但AI置信度不足(Gemma:{gemma_score}/40, DS:{ds_conf})，跳过第三止盈推送")
                elif pnl >= 5.0:
                    # 第二止盈：5%
                    last_alert = _alert_cooldown.get(f"tp2_sim_{code}", 0)
                    if time.time() - last_alert > ALERT_COOLDOWN_SECONDS:
                        msg = f"🏆 模拟盘第二止盈提醒！\n标单: {name}({code})\n现价: {curr_price}\n成本: {buy_price}\n盈亏: {pnl:.2f}%\n🛡️ 保护止损已上移至: ¥{dyn_stop:.2f}({dyn_label})\n⚠️ 建议大幅减仓或清仓"
                        push_decision("🧪模拟盘触发第二止盈", msg, code=code)
...
def precheck_buy_order(acc_type, code, buy_price_val, volume_val):
    ...
    if not _is_trade_time_now():
        # 只记录警告，不拦截
        now = datetime.now()
        logger.warning(f"⚠️ 非交易时段录单：{now.strftime('%Y-%m-%d %H:%M:%S')} | {acc_type} | {code} | {volume_val}股@{buy_price_val}")
...
async def async_execute_audit(code, m_ratio, m_vol):
    ...
    try:
        ...
        if is_held and h_info:
            ...
            keeper_loss = (pnl_pct <= -4.0) and (price < ma10) and (j_val > 30)
...
"""
        ...
        try:
            ...
            if r1_res:
                ...
                is_actionable = (is_buy or is_sell or is_hold) and not has_low_conf
        
                if is_actionable:
                    # 区分"首次买入"和"加仓"
                    is_new_buy = any(w in decision for w in ["轻仓买入", "买入"]) if decision else any(w in r1_res for w in ["轻仓买入", "买入"])
                    ...
                    action_tag = "买入" if is_buy else "卖出" if is_sell else "持有"
                    ...
                    if not blocked:
                        ...
                        # 仅"首次买入"记录（加仓不记录）
                        if is_new_buy and not is_add_pos:
...
async def async_main_loop():
    ...
    while True:
        try:
            # 1.5 每日自动复盘（15:10 后触发，当天只生成一次，不受交易窗口限制）
            now = datetime.now()
            ...
            # 4. 重置计数器（原版逻辑）
            consecutive_empty_cycles = 0
            log_terminal("系统", "🚀 开启新一轮【异步】审计（20只股票≈1~2秒）...")

            # 5. 【核心：异步批量审计（超级快）】
            await async_execute_audits(STOCKS, m_ratio, m_vol)
...
if __name__ == "__main__":
    ...
    t_main.start()
    log_terminal("策略引擎", "✅ 异步审计循环已启动（20只股票≈1~2秒）")
